...
export class TokenProvider implements Provider {
  name = "tokenProvider";
  description =
    "Fetches and injects DexScreener token metrics into agent context";
  dynamic = false;
  position = 0;
  private = false;
  ...
  async get(
    ...
    try {
      ...
      return {
        text: `Token ${pair.baseToken.symbol}: price $${pair.priceUsd}, 24h volume $${pair.volume.h24}, liquidity $${pair.liquidity.usd}.`,
        values: { tokenData: data },
        data: { tokenData: data },
      };
    } catch (error) {
      elizaLogger.error(
        `TokenProvider.get() failed for ${this.tokenAddress}:`,
        error
      );
      return { text: "" };
    }
