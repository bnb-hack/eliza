...
export class TrustScoreProvider implements Provider {
  name = "trustScoreProvider";
  description =
    "Calculates a trust score, risk level and trading advice for a token";
  dynamic = false;
  position = 10;
  private = false;
  ...
  async get(
    ...
    const tokenAddress = (state.values as any).tokenData?.dexScreenerData
    ...
    try {
      const { trustScore, riskLevel, tradingAdvice, reason } =
        await this.evaluateToken(tokenAddress);
      return {
        text: `Trust Score for ${tokenAddress}: ${trustScore.toFixed(
          2
        )} (${riskLevel}), Advice: ${tradingAdvice}. Reason: ${reason}`,
        values: { trustScore, riskLevel, tradingAdvice, reason },
        data: { trustScore, riskLevel, tradingAdvice, reason },
      };
    } catch (err) {
      elizaLogger.error(
        `TrustScoreProvider.get() failed for ${tokenAddress}:`,
        err
      );
      return { text: "" };
    }
