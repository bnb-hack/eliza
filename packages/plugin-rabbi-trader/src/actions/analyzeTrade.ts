...
export const analyzeTradeAction: Action = {
  ...
  handler: async (runtime, memory, state, params, callback) => {
    try {
      ...
      const prompt = `Analyze the following token data and provide a trading recommendation. YOUR ANY OUTPUT begin with '{' and end with '}'. Do not include any introductory or closing sentences; Return ONLY a valid JSON object exactly matching the following structure:
{
